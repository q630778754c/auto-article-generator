...
class PenguinAdapter(PlatformAdapter):
    platform = "penguin"
    ...
    async def publish(self, content: PublishContent, credential: OauthCredential | CookieCredential) -> PublishReceipt:
        ...
        if not isinstance(credential, CookieCredential):
            return PublishReceipt(success=False, fail_reason="企鹅号需要Cookie凭证")
        try:
            resp = await self._http.request(
                "POST", "https://om.qq.com/article/publish",
            ...
            if resp.status_code == 200 and data.get("code") == 0:
                item = data.get("data", {})
                return PublishReceipt(success=True, platform_article_id=str(item.get("article_id", "")))
